...
class FootballBettingAgent:
    ...
    def suggest_parlays(
        self, picks: List[BetRecommendation], max_legs: int = 3, max_parlays: int = 2
    ) -> List[Dict]:
        """Find the best multi-match parlay suggestions from daily picks.

        Selects uncorrelated picks from different leagues/matches with high
        confidence and unanimous/majority model agreement. Two independent 70%
        picks at parlay odds outperform singles for the same stake.

        Args:
            picks: Today's value picks (already filtered and sorted)
            max_legs: Maximum picks per parlay (2-3 recommended)
            max_parlays: Maximum number of parlay suggestions

        Returns:
            List of dicts with 'legs' (list of BetRecommendation),
            'combined_odds', 'combined_prob', 'expected_value'.
        """
        if len(picks) < 2:
            return []

        # Filter to high-quality parlay candidates:
        # - unanimous or majority model agreement
        # - confidence >= 55%
        # - not using fallback odds
        candidates = [
            p for p in picks
            if p.model_agreement in ("unanimous", "majority")
            and p.confidence >= 0.55
            and not p.used_fallback_odds
        ]

        if len(candidates) < 2:
            return []

        # Group by match_id to ensure one pick per match in parlay
        best_per_match: dict = {}
        for p in candidates:
            if p.match_id not in best_per_match:
                best_per_match[p.match_id] = p
            elif p.confidence > best_per_match[p.match_id].confidence:
                best_per_match[p.match_id] = p
        unique_picks = list(best_per_match.values())

        if len(unique_picks) < 2:
            return []

        # Sort by confidence descending
        unique_picks.sort(key=lambda p: p.confidence, reverse=True)

        # Generate parlays: greedily pick from different leagues
        parlays = []
        used_match_ids: set = set()

        for target_legs in range(min(max_legs, len(unique_picks)), 1, -1):
            legs = []
            leagues_used: set = set()
            for p in unique_picks:
                if p.match_id in used_match_ids:
                    continue
                # Prefer different leagues for diversification
                if p.league in leagues_used and len(legs) < target_legs - 1:
                    continue  # skip same-league unless we need to fill
                legs.append(p)
                leagues_used.add(p.league)
                if len(legs) >= target_legs:
                    break

            # If we didn't get enough legs from different leagues, allow same-league
            if len(legs) < 2:
                legs = []
                for p in unique_picks:
                    if p.match_id not in used_match_ids:
                        legs.append(p)
                        if len(legs) >= target_legs:
                            break
            ...
            for leg in legs:
                ...
                combined_prob *= leg.predicted_probability
